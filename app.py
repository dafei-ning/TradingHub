...
from flask_script     import Manager, Shell #命令行直接传运行参数，没什么用
from flask            import Flask, url_for, redirect, request, render_template, session, flash
#Flask 必备， url_for是生产视图函数url的，redirect是跳转用，request是读取网络内容用，
#render_template是给jinja模板渲染用，session身份管理 暂时用不到， flash返回提示信息用
from datetime         import datetime #时间包暂时用不到
from flask_sqlalchemy import SQLAlchemy #数据库ORM 必须
from sqlalchemy       import distinct, func #数据库查询功能扩展
from binance.client   import Client # 币安的api
from urllib           import parse, request #爬网络内容用的
from config           import config #把secret从文件内读取
import json # json的操作包
...
app = Flask(__name__)
manager = Manager(app)
bottstrap = Bootstrap(app)

# 实例参数部分设置

app.config.from_object(config['default']) # 读入config中的default类参数（就是development类）
...
app.config['SQLALCHEMY_TRACK_MODIFICATIONS'] = True # 忘记
